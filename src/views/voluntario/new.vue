<template>
   <main class="pb-40">
      <page-heading title="Nuevo Voluntario" back-route="/voluntario" :breadcrumbs="breadcrumbs" />
      <form-section>
...
export default class NewVoluntarioPage extends Vue {
  ...
  @VoluntarioModel.Action('store') createVoluntario!: ({ voluntario, vm }: { voluntario: any; vm: any }) => ActionMethod;
  ...
  async onSubmit() {
    ...
    if (isValid) {
      try {
        ...
        await this.createVoluntario({ voluntario: this.form, vm: this });
