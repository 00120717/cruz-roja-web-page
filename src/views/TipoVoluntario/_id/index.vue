<template>
  <main class="pb-40">
    <page-heading title="Detalle del Tipo de Voluntario" back-route="/tipoVoluntario" :breadcrumbs="breadcrumbs">
      <template slot="actions">
        <custom-button
          ...
          size="small"
          class="mr-2"
          :to="`/tipoVoluntario/${tipoVoluntario.id}/edit`"
        >
          <template #icon>
            <svg
              class="w-4 h-4 mr-2"
      ...
      <div class="overflow-hidden bg-white border border-gray-100 shadow sm:rounded-lg">
        <div class="px-4 py-5 border-b border-gray-100 sm:px-6">
          <h3 class="text-lg font-medium leading-6 text-gray-900">
            Tipo de Voluntario #{{ tipoVoluntario.id }}
          </h3>
          <p class="max-w-2xl mt-1 text-sm leading-5 text-gray-500">
            Información general del Tipo de Voluntario {{ tipoVoluntario.tipo }}.
          </p>
        </div>
        <div>
          <dl>
    ...
    <delete-item
      ...
      :title="`Tipo de voluntario ${tipoVoluntario.tipo}`"
      @update:show="showDeleteModal = false"
      @action="deleteItem"
    />
    <confirmation-modal
      :show="showConfirmationModal"
      title="Tipo de Voluntario Eliminado"
      @update:show="showConfirmationModal = false"
      @action="redirectBack"
    />
    <loading :active="isTipoVoluntarioLoading" :is-full-page="false" />
  </main>
</template>
...
export default class ShowTipoVoluntarioPage extends Vue {
  ...
  @TiposVoluntarios.Action('destroy') deleteTipoVoluntario!: ({ id, vm }: { id: number; vm: any }) => ActionMethod

  async mounted() {
    const { id } = this.$route.params;
    await this.fetchTipoVoluntario({ id: +id, vm: this });
    if (!this.tipoVoluntario.id) {
      this.redirectBack();
    }
  }

  async deleteItem() {
    const id = Number(this.tipoVoluntario?.id);
    await this.deleteTipoVoluntario({ id, vm: this });
