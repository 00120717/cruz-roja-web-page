<template>
  <main class="pb-40">
    <page-heading title="Detalle Hospital" back-route="/hospital" :breadcrumbs="breadcrumbs">
      ...
      <div class="overflow-hidden bg-white border border-gray-100 shadow sm:rounded-lg">
        <div class="px-4 py-5 border-b border-gray-100 sm:px-6">
          ...
          <p class="max-w-2xl mt-1 text-sm leading-5 text-gray-500">
            Información general del Hospital {{ hospital.nombreHospital }}.
          </p>
        </div>
        <div>
          <dl>
    ...
    <delete-item
      ...
      :title="`Hospital ${hospital.nombrehospital}`"
    ...
    <confirmation-modal
      ...
      @action="redirectBack"
    />
    <loading :active="isHospitalLoading" :is-full-page="false" />
  </main>
</template>
...
export default class ShowHospitalPage extends Vue {
  ...
  @HospitalModel.Action('destroy') deleteHospital!: ({ id, vm }: { id: number; vm: any }) => ActionMethod;

  async mounted() {
    const { id } = this.$route.params;
    await this.fetchHospital({ id: +id, vm: this });
    if (!this.hospital.id) {
      this.redirectBack();
    }
  }
  ...
  async deleteItem() {
    const id = Number(this.hospital?.id);
    await this.deleteHospital({ id, vm: this });
