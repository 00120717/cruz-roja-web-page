  ...
  <main class="pb-40">
    ...
    >
      <template slot="actions">
        <custom-button title="Nuevo Hospital" color="secondary" to="/hospital/new">
          <template #icon>
            <svg
              class="w-4 h-4 mr-2"
              fill="none"
...
export default class HospitalPage extends Vue {
  selectedItems: Array<string | number> = []
  showDeleteModal = false
  showDeleteCompleted = false
  breadcrumbs: Breadcrumb[] = [{ name: 'Administración' }, { name: 'Hospital' }]
  ...
  selectedItem = 0
  @HospitalModel.State('hospitales') hospitales!: Hospital[];
  @HospitalModel.State('meta') meta!: Meta;
  @HospitalModel.State('isLoading') isHospitalLoading!: boolean;
  @HospitalModel.Action('fetch') fetchHospitales!: ({ filters, vm }: { filters: Filters; vm: any }) => AxiosResponse;

  async mounted() {
    await this.fetchHospitales({ filters: this.filters, vm: this });
  }
  ...
  selectAllCurrent(value: boolean) {
    if (value) {
      this.hospitales.forEach((item: any) => {
