<template>
  <main class="pb-40">
    <page-heading title="Detalle paciente" back-route="/paciente" :breadcrumbs="breadcrumbs">
          ...
          <dl>
            ...
            <div class="px-4 py-5 bg-gray-50 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd
                class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2"
              >
                <active-indicator :status="Boolean(paciente.estadoPersona)" />
              </dd>
            </div>
          </dl>
        </div>
    ...
    <delete-item
      ...
      :title="`${paciente.firstName} ${paciente.lastName}`"
    ...
    <confirmation-modal
      ...
      @action="redirectBack"
    />
    <loading :active="isPacienteLoading" :is-full-page="false" />
  </main>
</template>
...
export default class ShowPacientePage extends Vue {
  ...
  @PacienteModel.Action('show') fetchPaciente!: ({ id, vm }: { id: string; vm: any }) => ActionMethod
  @PacienteModel.Action('destroy') deletePaciente!: ({ id, vm }: { id: string; vm: any }) => ActionMethod

  async mounted() {
    const { id } = this.$route.params;
    await this.fetchPaciente({ id: String(+id), vm: this });
    if (!this.paciente.id) {
      this.redirectBack();
    }
  }
  ...
  async deleteItem() {
    await this.deletePaciente({ id: String(this.paciente?.id), vm: this });
