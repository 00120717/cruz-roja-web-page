  ...
  <main class="pb-40">
    ...
    >
      <template slot="actions">
        <custom-button title="Nueva modalidad" color="secondary" to="/modalidad/new">
          <template #icon>
            <svg
              class="w-4 h-4 mr-2"
              fill="none"
...
export default class ModalidadPage extends Vue {
  ...
  selectedItem = 0
  @ModalidadModel.State('modalidades') modalidades!: Modalidad[];
  @ModalidadModel.State('meta') meta!: Meta;
  @ModalidadModel.State('isLoading') isModalidadLoading!: boolean;
  @ModalidadModel.Action('fetch') fetchModalidades!: ({ filters, vm }: { filters: Filters; vm: any }) => AxiosResponse;

  async mounted() {
    await this.fetchModalidades({ filters: this.filters, vm: this });
  }
