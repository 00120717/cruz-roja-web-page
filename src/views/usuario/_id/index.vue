<template>
  <main class="pb-40">
    <page-heading title="Detalle usuario" back-route="/usuario" :breadcrumbs="breadcrumbs">
      <template slot="actions">
        <custom-button
          v-if="usuario.rol.tipo !== 'superadmin'"
          title="Editar"
          color="white"
          size="small"
          class="mr-2"
        ...
        <custom-button
          v-if="usuario.rol.tipo !== 'superadmin' && usuario.rol.tipo !== 'admin'"
          title="Eliminar"
          color="secondary"
          size="small"
        ...
        <div>
          <dl>
            ...
            <div class="px-4 py-5 bg-white sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              >
                {{ usuario.phoneNumber ? usuario.phoneNumber : 'N/A' }}
              </dd>
            </div>
            <div class="px-4 py-5 bg-gray-50 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium leading-5 text-gray-500">
              ...
              <dd
                class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2"
              >
                {{ usuario.altPhoneNumber ? usuario.altPhoneNumber : 'N/A' }}
              </dd>
            </div>
            <div class="px-4 py-5 bg-white sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium leading-5 text-gray-500">
            ...
            <div class="px-4 py-5 bg-gray-50 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              ...
              <dd
                class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2"
              >
                {{ usuario.rol && usuario.rol.nombre ? usuario.rol.nombre : 'N/A' }}
              </dd>
            </div>
            <div class="px-4 py-5 bg-white sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium leading-5 text-gray-500">
              ...
              <dd
                class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2"
              >
                {{ usuario.rol && usuario.rol.tipo ? usuario.rol.tipo : 'N/A' }}
              </dd>
            </div>
            <div class="px-4 py-5 bg-gray-50 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
              <dt class="text-sm font-medium leading-5 text-gray-500">
              ...
              <dd class="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                <active-indicator :status="Boolean(usuario.status)" />
              </dd>
            </div>
          </dl>
        </div>
    ...
    <confirmation-modal
      ...
      @action="redirectBack"
    />
    <loading :active="isUsuarioLoading" :is-full-page="false" />
  </main>
</template>
...
export default class ShowUsuarioPage extends Vue {
  ...
  @UsuarioModel.State('usuario') usuario!: Usuario
  @UsuarioModel.Action('show') fetchUsuario!: ({ id, vm }: { id: number; vm: any }) => ActionMethod
  @UsuarioModel.Action('destroy') deleteUsuario!: ({ id, vm }: { id: number; vm: any }) => ActionMethod

  async mounted() {
    const { id } = this.$route.params;
    await this.fetchUsuario({ id: +id, vm: this });
    if (!this.usuario.id) {
      this.redirectBack();
    }
  }
  ...
  async deleteItem() {
    ...
    await this.deleteUsuario({ id, vm: this });
