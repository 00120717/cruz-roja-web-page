  ...
  <main class="pb-40">
    ...
    >
      <template slot="actions">
        <custom-button title="Nuevo rol" color="secondary" to="/rol/new">
          <template #icon>
            <svg
              class="w-4 h-4 mr-2"
              fill="none"
      ...
      >
        <template slot="items">
          <table-item
            ...
            :item="rol.id"
            :selected="selectedItems"
            :edit="rol.type !== 'superadmin' && rol.type !== 'admin'"
            :remove="rol.type !== 'superadmin' && rol.type !== 'admin'"
            page="rol"
...
export default class RolPage extends Vue {
  ...
  selectedItem = 0
  @RolModel.State('roles') roles!: Rol[];
  @RolModel.State('meta') meta!: Meta;
  @RolModel.State('isLoading') isRolLoading!: boolean;
  @RolModel.Action('fetch') fetchRoles!: ({ filters, vm }: { filters: Filters; vm: any }) => AxiosResponse;
  @RolModel.Action('destroy') deleteRol!: ({ id, vm }: { id: number; vm: any }) => AxiosResponse;
  ...
  async deleteComplete() {
    if (this.selectedItem !== 0) {
      await this.deleteRol({ id: this.selectedItem, vm: this });
      await this.fetchRoles({ filters: this.filters, vm: this });
