<template>
  <main class="pb-40">
    <page-heading title="Nuevo Vehiculo" :back-route="`/vehiculo/${$route.params.id}`" :breadcrumbs="breadcrumbs" />
    <form-section>
...
export default class EditVehiculoPage extends Vue {
  ...
  async mounted() {
    try {
      await this.fetchVehiculo({ id: +this.$route.params.id, vm: this });
      this.setCurrentVehiculo();
    } catch (e) {
  ...
  async onSubmit() {
    ...
    if (isValid) {
      try {
        await this.updateVehiculo({ vehiculo: this.form, vm: this });
