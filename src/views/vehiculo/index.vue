  ...
  <main class="pb-40">
    ...
    >
      <template slot="actions">
        <custom-button title="Nuevo Vehiculo" color="secondary" to="/vehiculo/new">
          <template #icon>
            <svg
              class="w-4 h-4 mr-2"
              fill="none"
...
export default class VehiculoPage extends Vue {
  ...
  breadcrumbs: Breadcrumb[] = [{ name: 'Administración' }, { name: 'Vehiculo' }]
  ...
  selectedItem = 0
  @VehiculoModel.State('vehiculos') vehiculos!: Vehiculo[];
  @VehiculoModel.State('meta') meta!: Meta;
  @VehiculoModel.State('isLoading') isVehiculoLoading!: boolean;
  @VehiculoModel.Action('fetch') fetchVehiculos!: ({ filters, vm }: { filters: Filters; vm: any }) => AxiosResponse;

  async mounted() {
    await this.fetchVehiculos({ filters: this.filters, vm: this });
  }
