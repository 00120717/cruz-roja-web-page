<template>
  <main class="pb-40">
    <page-heading title="Nueva Emergencia" :back-route="`/emergencia/${$route.params.id}`" :breadcrumbs="breadcrumbs" />
    <form-section>
    ...
    <div class="w-full">
      <div class="flex items-center justify-end">
        <custom-button type="submit" class="ml-2" title="Guardar" :loading="isEmergenciaLoading" @click.prevent="onSubmit" />
      </div>
    </div>
  </main>
</template>
...
export default class EditEmergenciaPage extends Vue {
  ...
  @EmergenciaModel.Action('update') updateEmergencia!: ({ emergencia, vm }: { emergencia: any; vm: any }) => ActionMethod;
  @EmergenciaModel.Action('show') fetchEmergencia!: ({ id, vm }: { id: number; vm: any }) => ActionMethod;

  async mounted() {
    try {
      await this.fetchEmergencia({ id: +this.$route.params.id, vm: this });
      await this.fetchTipoEmergenciaList(this);
      this.setCurrentEmergencia();
    } catch (e) {
      (this as any).$snotify.error('Ha ocurrido un error');
    }
  }

  setCurrentEmergencia() {
    this.form = {
      ...this.emergencia,
    };
    this.form.tipoEmergenciaId = this.emergencia?.tipoEmergencia?.id ?? 0;
  }
  ...
  async onSubmit() {
    ...
    if (isValid) {
      try {
        await this.updateEmergencia({ emergencia: this.form, vm: this });
