  ...
  <main class="pb-40">
    ...
    >
      <template slot="actions">
        <custom-button title="Nueva Emergencia" color="secondary" to="/emergencia/new">
          <template #icon>
            <svg
              class="w-4 h-4 mr-2"
              fill="none"
...
const EmergenciasModel = namespace('emergencia');
...
export default class EmergenciaPage extends Vue {
  ...
  breadcrumbs: Breadcrumb[] = [{ name: 'Administración' }, { name: 'Emergencia' }]
  ...
  headers: Array<object> = [
    ...
    {
      name: 'Descripcion Emergencia',
      key: 'emergenciaDescripcion',
    },
    {
  ...
  selectedItem = 0
  @EmergenciasModel.State('emergencias') emergencias!: Emergencia[];
  @EmergenciasModel.State('meta') meta!: Meta;
  @EmergenciasModel.State('isLoading') isEmergenciaLoading!: boolean;
  @EmergenciasModel.Action('fetch') fetchEmergencias!: ({ filters, vm }: { filters: Filters; vm: any }) => AxiosResponse;
  ...
  selectAllCurrent(value: boolean) {
    if (value) {
      this.emergencias.forEach((item: any) => {
