  ...
  <main class="pb-40">
    ...
    >
      <template slot="actions">
        <custom-button title="Nueva seccional" color="secondary" to="/seccional/new">
          <template #icon>
            <svg
              class="w-4 h-4 mr-2"
              fill="none"
    ...
    <section>
      ...
      >
        <template slot="items">
          <table-item
            v-for="(seccional, index) in seccionales"
            :key="index"
            :item="seccional.id"
            :selected="selectedItems"
            :remove="false"
...
export default class SeccionalPage extends Vue {
  selectedItems: Array<string | number> = []
  showDeleteModal = false
  showDeleteCompleted = false
  breadcrumbs: Breadcrumb[] = [{ name: 'Administración' }, { name: 'Seccional' }]
  ...
  selectedItem = 0
  @SeccionalModel.State('seccionales') seccionales!: Seccional[];
  @SeccionalModel.State('meta') meta!: Meta;
  @SeccionalModel.State('isLoading') isSeccionalLoading!: boolean;
  @SeccionalModel.Action('fetch') fetchSeccionales!: ({ filters, vm }: { filters: Filters; vm: any }) => AxiosResponse;

  async mounted() {
    await this.fetchSeccionales({ filters: this.filters, vm: this });
  }
  ...
  selectAllCurrent(value: boolean) {
    if (value) {
      this.seccionales.forEach((item: any) => {
